...
func (c *RiotClient) GetShards() (*[]Shard, error) {
	// Performs the http request to Riots API to retrieve a list of shards
...
func (c *RiotClient) GetShardStatus(region string) (*ShardStatus, error) {
	// Performs the http request to Riots API to retrieve a specific shard status
	resBody, err := c.riotRequest("/shards/"+strings.ToLower(region), region, nil)
	if err != nil {
		return nil, err
	}
